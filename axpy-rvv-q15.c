		  ...
		  int16_t *y, int n, int16_t alpha)
{
	...
	// My solution:
	for(;n > 0;) {
		size_t len = vsetvl_e16m1(n);

		/* set a and b */
		vint16m1_t va = vle16_v_i16m1(a, len);
		vint16m1_t vb = vle16_v_i16m1(b, len);

		/* wide multiply: alpha * b */
		vint32m2_t vmul = vwmul_vx_i32m2(vb, alpha, len);

		/* wide add: a + (alpha * b) */
		vint32m2_t vsum = vwadd_wv_i32m2(vmul, va, len);

		/* saturate to narrow to Q15 */
		vint16m1_t vy = vnclip_wx_i16m1(vsum, 0, len);

		/* store */
		vse16_v_i16m1(y, vy, len);
